...
(function (global, doc) {
    ...
    function BookingFlow(mount) {
        ...
        // Check if logged-in patient exists
        var db = global.LumoraDB || null;
        var patSession = db && db.getPatientSession ? db.getPatientSession() : null;
        if (patSession) {
            state.patient.name = patSession.name || '';
            state.patient.phone = (patSession.phone || '').replace('+91', '').trim();
            state.patient.dialCode = '+91';
            state.patient.age = String(patSession.age || '');
            state.patient.email = patSession.email || '';

            var allAppts = db.getAppointments ? db.getAppointments() : [];
            var pastAppts = allAppts.filter(function (a) {
                return a.patientId === patSession.refId ||
                       (a.patientEmail && a.patientEmail.toLowerCase() === patSession.email.toLowerCase()) ||
                       (patSession.phone && a.patientPhone && a.patientPhone.replace(/\D/g, '') === patSession.phone.replace(/\D/g, ''));
            });

            if (pastAppts.length > 0) {
                state.isReturningPatient = true;
                var lastAppt = pastAppts[pastAppts.length - 1];
                state.doctorId = lastAppt.doctorId || 'anupam-purwar';
    ...
    function BookingDetails(state, go) {
        ...
        if (state.isReturningPatient) {
            var retBanner = el('div', {
                style: 'background:rgba(36,163,177,0.12); border:1px solid rgba(36,163,177,0.3); border-radius:12px; padding:12px 16px; margin-bottom:18px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;'
            }, [
                el('div', { style: 'display:flex; align-items:center; gap:10px;' }, [
                    el('span', { style: 'font-size:20px;', text: '👋' }),
                    el('div', {}, [
                        el('strong', { style: 'color:var(--bk-dark); font-size:14px;', text: 'Welcome back, ' + (state.patient.name || 'Patient') + '!' }),
                        el('div', { style: 'font-size:12px; color:var(--bk-muted);', text: 'Doctor & Service pre-filled from your previous visit. Select your date & time below.' })
                    ])
                ]),
                el('button', {
                    type: 'button',
                    ...
                    onClick: function () { go('doctor'); }
                }, [el('span', { text: 'Change Doctor' })])
            ]);
            main.appendChild(retBanner);
        }
        ...
        function renderSlots() {
            ...
            var nextStep = (state.isReturningPatient && state.patient.name && state.patient.phone) ? 'confirm' : 'details';
            timeSide.appendChild(TimeSlotSelector(state, function () { go(nextStep); }));
        }
    ...
    function ConfirmScreen(state, go) {
        var main = el('div', { class: 'bk-panel__main' });
        var panel = el('div', { class: 'bk-panel' }, [
            DoctorAside(state, { onBack: function () { go('schedule'); }, backLabel: 'Change date/time' }),
            main
        ]);

        var doctor = API.getDoctor(state.doctorId) || { name: 'Assigned Specialist', specialization: 'Dentistry', image: 'assets/img/gen_team-image-1.jpg' };
        var service = API.getService(state.serviceId) || { name: 'Dental Consultation', duration: 30 };
        var p = state.patient || {};

        main.appendChild(el('h2', { class: 'bk-main__title', text: 'Cross-Check & Confirm' }));
        main.appendChild(el('p', {
            class: 'bk-main__para',
            text: 'Please review all your appointment details below. You can change any item before final confirmation.'
        }));

        var reviewCardsWrap = el('div', { style: 'display:flex; flex-direction:column; gap:14px; margin:20px 0;' });

        // Section 1: Doctor & Service
        var secDoctor = el('div', {
            style: 'background:rgba(1,31,35,0.04); border:1px solid var(--bk-line); border-radius:14px; padding:16px 18px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;'
        }, [
            el('div', { style: 'display:flex; align-items:center; gap:14px;' }, [
                el('img', { src: doctor.image, alt: doctor.name, style: 'width:46px; height:46px; border-radius:50%; object-fit:cover; border:2px solid var(--bk-primary);' }),
                el('div', {}, [
                    el('div', { style: 'font-size:11px; text-transform:uppercase; letter-spacing:0.06em; color:var(--bk-primary); font-weight:600;', text: 'Consultant & Service' }),
                    el('h3', { style: 'margin:2px 0; font-size:15px; color:var(--bk-dark); font-weight:600;', text: doctor.name }),
                    el('div', { style: 'font-size:13px; color:var(--bk-muted);', text: service.name + ' (' + service.duration + ' min)' })
                ])
            ]),
            el('button', {
                type: 'button',
                ...
                onClick: function () { go('doctor'); }
            }, [el('span', { text: 'Change Doctor ✏️' })])
        ]);

        // Section 2: Date & Time Schedule
        var secSchedule = el('div', {
            style: 'background:rgba(1,31,35,0.04); border:1px solid var(--bk-line); border-radius:14px; padding:16px 18px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;'
        }, [
            el('div', { style: 'display:flex; align-items:center; gap:14px;' }, [
                el('div', { style: 'width:42px; height:42px; border-radius:10px; background:var(--bk-tint); display:grid; place-items:center; color:var(--bk-primary);' }, [
                    icon(ICONS.calendar, 20)
                ]),
                el('div', {}, [
                    el('div', { style: 'font-size:11px; text-transform:uppercase; letter-spacing:0.06em; color:var(--bk-primary); font-weight:600;', text: 'Scheduled Slot' }),
                    el('h3', { style: 'margin:2px 0; font-size:15px; color:var(--bk-dark); font-weight:600;', text: state.dateIso ? API.formatDateLong(parseIso(state.dateIso)) : 'Date not set' }),
                    el('div', { style: 'font-size:13px; color:var(--bk-muted);', text: (state.slot ? state.slot.label : '') + ' (' + timezoneLabel(state.timezone) + ')' })
                ])
            ]),
            el('button', {
                type: 'button',
                class: 'bk-btn bk-btn--ghost',
                style: 'font-size:12px; padding:6px 12px; border-radius:8px;',
                onClick: function () { go('schedule'); }
            }, [el('span', { text: 'Change Date/Time ✏️' })])
        ]);

        // Section 3: Patient Information
        var secPatient = el('div', {
            style: 'background:rgba(1,31,35,0.04); border:1px solid var(--bk-line); border-radius:14px; padding:16px 18px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;'
        }, [
            el('div', { style: 'display:flex; align-items:center; gap:14px;' }, [
                el('div', { style: 'width:42px; height:42px; border-radius:10px; background:var(--bk-tint); display:grid; place-items:center; color:var(--bk-primary);' }, [
                    icon('<path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>', 20)
                ]),
                el('div', {}, [
                    el('div', { style: 'font-size:11px; text-transform:uppercase; letter-spacing:0.06em; color:var(--bk-primary); font-weight:600;', text: 'Patient Information' }),
                    el('h3', { style: 'margin:2px 0; font-size:15px; color:var(--bk-dark); font-weight:600;', text: p.name || 'Patient' }),
                    el('div', { style: 'font-size:13px; color:var(--bk-muted);', text: (p.dialCode || '+91') + ' ' + (p.phone || '') + (p.age ? ' • Age: ' + p.age : '') + (p.notes ? ' • Note: ' + p.notes : '') })
                ])
            ]),
            el('button', {
                type: 'button',
            ...
            }, [el('span', { text: 'Edit Details ✏️' })])
        ]);

        reviewCardsWrap.appendChild(secDoctor);
        reviewCardsWrap.appendChild(secSchedule);
        reviewCardsWrap.appendChild(secPatient);
        main.appendChild(reviewCardsWrap);

        var alert = el('p', { class: 'bk-alert', role: 'alert' }, [icon(ICONS.alert), el('span', { text: '' })]);
        main.appendChild(alert);

        var confirm = el('button', { type: 'button', class: 'bk-btn bk-btn--primary', style: 'width:100%; justify-content:center; padding:15px;' }, [
            el('span', { text: 'Confirm & Book Appointment' }), icon(ICONS.check, 17)
        ]);
        ...
        confirm.addEventListener('click', function () {
            ...
            API.submitBooking({
                doctor: doctor,
                service: service,
                startIso: state.slot ? state.slot.start : new Date().toISOString(),
                ...
                confirm.firstChild.textContent = 'Confirm & Book Appointment';
                alert.lastChild.textContent = 'We could not complete your booking. Please try again, or call us on ' + CLINIC.phone + '.';
                alert.classList.add('is-visible');
            });
        });

        main.appendChild(el('div', { class: 'bk-actions', style: 'margin-top:16px;' }, [confirm]));
    ...
    function BookingSuccess(state) {
        ...
        var waBtn = el('a', {
            class: 'bk-btn',
            style: 'background:#25d366; color:#fff; border-color:#25d366; font-weight:600; display:inline-flex; align-items:center; gap:8px;',
            href: waUrl,
            target: '_blank'
        }, [
            ...
            el('span', { text: 'Send Confirmation on WhatsApp' })
        ]);

        var status = el('p', { class: 'bk-success__status', role: 'status' }, [el('span', { text: '' })]);
        var addBtn = el('button', {
            type: 'button', class: 'bk-btn bk-btn--ghost',
            onClick: function () { addToCalendar(state, status.firstChild); }
        ...
        return el('div', { class: 'bk-success' }, [
            ...
            card,
            el('div', { class: 'bk-actions bk-actions--center', style: 'margin-bottom:12px;' }, [waBtn, addBtn]),
            status,
            el('div', { class: 'bk-actions bk-actions--center' }, [
                el('a', { class: 'bk-btn bk-btn--ghost', href: 'tel:+' + CLINIC.phoneE164 },
                    [icon(ICONS.phone, 17), el('span', { text: 'Call Clinic' })]),
