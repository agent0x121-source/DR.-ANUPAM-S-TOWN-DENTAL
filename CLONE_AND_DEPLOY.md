# Dr. Anupam's Town Dental — Clone & Deploy (the reliable one-shot)
...
**Source of truth (public):** `https://github.com/toprmrproducer/dr-patil-clinic`
**Live reference (what the result must look like):** `https://toprmrproducer.github.io/dr-patil-clinic/`
...
```bash
git clone https://github.com/toprmrproducer/dr-patil-clinic.git
cd dr-patil-clinic
```
...
- Browser: open `https://github.com/toprmrproducer/dr-patil-clinic/archive/refs/heads/main.zip`, unzip,
  and `cd` into the unzipped `dr-patil-clinic-main` folder.
- macOS/Linux one-liner: `curl -L -o lumora.zip https://github.com/toprmrproducer/dr-patil-clinic/archive/refs/heads/main.zip && unzip lumora.zip && cd dr-patil-clinic-main`
- Windows PowerShell: `Invoke-WebRequest https://github.com/toprmrproducer/dr-patil-clinic/archive/refs/heads/main.zip -OutFile lumora.zip; Expand-Archive lumora.zip -Force; cd dr-patil-clinic-main\dr-patil-clinic-main`
...
1. On GitHub, **Fork** `toprmrproducer/dr-patil-clinic` into your own account (button top-right), OR
...
1. **Brand name:** find `Dr. Anupam's Town Dental` (and standalone `Lumora`) across all `.html` → your business name.
...
4. **Phone / WhatsApp:** find `9307512816` (and `918126643459` if present) → your number (the lead
   form opens a prefilled WhatsApp on submit — handled by the inline `lumoraLead` script before
   `</body>` on each index page).
5. **Email:** find `dranupamstowndental111@gmail.com` → your email.
