# Dr. Patil’s Dental Care — Clone & Deploy (the reliable one-shot)
...
1. **Brand name:** find `Dr. Patil’s Dental Care` (and standalone `Lumora`) across all `.html` → your business name.
...
4. **Phone / WhatsApp:** find `9307512816` (and `919146029424` if present) → your number (the lead
   form opens a prefilled WhatsApp on submit — handled by the inline `lumoraLead` script before
   `</body>` on each index page).
5. **Email:** find `hello@drpatilsdentalcare.com` → your email.
